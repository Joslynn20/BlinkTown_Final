...
public class AdminController {
	...
	public ModelAndView updateProduct(String productCode, String category) {
	...
	/**
	 * 관리자-회원목록+회원숫자count 위치 : #tab1 / 출력 : #usersListTable / 이벤트 클릭: #usersTab1
	 */
	@ResponseBody
	...
	public Map<String, Object> usersList(@PathVariable String isMember) {
		...
		Integer isMem = null;
		if (isMember.equals("all"))
			isMem = null;
		else if (isMember.equals("normal"))
			isMem = 0;
		else if (isMember.equals("member"))
			isMem = 1;
	...
	/////////////////////////////////////////////////
	/**
	 * 관리자페이지-주문목록 출력 ajax 위치 : #tab3 / class="tbl-content" id="ordersList" / 이벤트 클릭
	 * : #ordersTab3
	 */
	@ResponseBody
	...
	/**
	 * 관리자페이지-챠트 에 넣을 데이터 출력 ajax 위치 : #tab4 / ajax로 데이터 호출 후 챠트에 넣음 / 이벤트 클릭 :
	 * #statsTab4 전체 주문 매출 : allStats (totalPrice) 월별 주문 매출 : monthStats(month ,
	 * totalPrice) 앨범 주문 매출 : albumStats(albumTotalPrice, albumTotalQty)
	 */
	@ResponseBody
